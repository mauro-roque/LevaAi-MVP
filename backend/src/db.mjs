...
/** Abre SQLite para demonstração ou PostgreSQL quando DATABASE_URL está definido. */
export async function openDatabase({
  url,
  file = resolve("data/leva-ai.sqlite"),
} = {}) {
  let query, close;
  if (url) {
  ...
  const schema = await readFile(
    new URL("../migrations/001_mvp.sql", import.meta.url),
    "utf8",
  );
