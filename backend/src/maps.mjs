...
/** Endereços fixos usados como alternativa para a demonstração. */
...
/** Calcula a distância aproximada entre dois pontos, em quilômetros. */
...
/** Cria os adaptadores de busca de endereço e rota. */
  ...
  return {
    async search(query) {
      ...
      const operation = queue.then(async () => {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, Math.max(0, 1100 - (Date.now() - lastCall))),
        );
        lastCall = Date.now();
        ...
        const data = (await get(url)).map((p) => ({
    ...
    async route(origin, destination, demo) {
      ...
      if (demo) {
          ...
          demoPlaces.some((p) => distance(p, origin) < 0.01) &&
            demoPlaces.some((p) => distance(p, destination) < 0.01),
          "Use os endereços de exemplo para a rota demonstrativa.",
        );
        return {
          distanceKm: Math.round(distance(origin, destination) * 1.4 * 10) / 10,
          durationMinutes: 25,
          demo: true,
          coordinates: [
      ...
      return {
        distanceKm: Math.ceil(r.distance / 100) / 10,
