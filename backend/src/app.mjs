...
/** Lê as variáveis locais e valida o modo de execução da API Node. */
export function configuration(env = process.env) {
  const demo = env.DEMO_MODE !== "false";
  ensure(
    demo || (env.JWT_SECRET?.length >= 32 && env.DATABASE_URL),
    "Modo real exige DATABASE_URL e JWT_SECRET de pelo menos 32 caracteres.",
  );
  return {
    demo,
    ...
    staticDir:
      env.STATIC_DIR || resolve(projectDir, "frontEnd/leva_ai/build/web"),
  };
}

/** Insere os dados de demonstração uma única vez no banco vazio. */
  ...
  await db.transaction(async () => {
    for (const u of [
      ...
      await db.query(
        ...
        [u[0], u[1], u[2], password, u[3], "(11) 99999-0000", now()],
      );
    const sample = [
      [
        "demo-van",
    ...
    for (const v of sample)
      await db.query(
        "INSERT INTO vehicles(id,provider_id,data) VALUES($1,$2,$3)",
        [
          v[0],
          v[1],
          JSON.stringify({
...
/** Cria o manipulador HTTP local com as mesmas regras do Worker publicado. */
export function createApp(db, config, overrides = {}) {
  const maps = overrides.maps || createMaps(config),
  ...
  async function session(user) {
    ...
    await db.query(
      ...
      [sid, user.id, new Date(Date.now() + 28800000).toISOString()],
    );
    return {
      user: publicUser(user),
      token: signToken(user, config.secret, sid),
    };
  }
  ...
  async function compatible(vehicle, input) {
    ...
    if (
      !provider?.available ||
      !vehicle.active ||
      v.capacityKg < input.weightKg ||
      v.volumeM3 < input.volumeM3 ||
      v.helpers < input.helpers ||
      distance(v.base, input.origin) > v.radiusKm
    )
      return false;
    const rows = await db.query(
      "SELECT * FROM bookings WHERE vehicle_id=$1 AND service_date=$2",
  ...
  async function route(req, path, b, user) {
    ...
    if (method === "POST" && path === "/api/auth/register") {
      const name = text(b.name, "Nome", 2, 150),
        email = text(b.email, "E-mail", 5, 150).toLowerCase();
      ensure(
        /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email),
        "Informe um e-mail válido.",
      );
      ensure(["cliente", "prestador"].includes(b.role), "Escolha seu perfil.");
      const password = text(b.password, "Senha", 8, 128),
        phone = text(b.phone, "Telefone", 10, 20);
      ensure(
      ...
      await db.query(
        "INSERT INTO users(id,name,email,password_hash,role,phone,created_at) VALUES($1,$2,$3,$4,$5,$6,$7)",
        [u.id, name, email, await hashPassword(password), b.role, phone, now()],
      );
      return session(u);
    }
    if (method === "POST" && path === "/api/auth/login") {
      const email = text(b.email, "E-mail", 3, 150).toLowerCase(),
        password = text(b.password, "Senha", 1, 128);
    ...
    if (method === "PATCH" && path === "/api/me/availability") {
      role(user, "prestador");
      ensure(typeof b.available === "boolean", "Disponibilidade inválida.");
    ...
    if (method === "GET" && path === "/api/vehicles") {
      role(user, "prestador");
      return (
        await db.query("SELECT * FROM vehicles WHERE provider_id=$1", [user.id])
      ).map(parsed);
    }
    if (method === "POST" && path === "/api/vehicles") {
      ...
      await db.query(
        ...
        [vehicleId, user.id, data.active ? 1 : 0, JSON.stringify(data)],
      );
      return {
        id: vehicleId,
        provider_id: user.id,
    ...
    if (method === "POST" && path === "/api/quotes") {
      ...
      const vehicles = (
          await db.query("SELECT * FROM vehicles WHERE active=1")
        ).map(parsed),
        quotes = [];
      for (const vehicle of vehicles) {
        ...
        const transportCents = Math.round(route.distanceKm * v.pricePerKmCents),
          helpersCents = input.helpers * v.helperPriceCents;
        const data = {
          input,
          route,
          ...
          providerDistanceKm:
            Math.round(distance(v.base, input.origin) * 10) / 10,
          transportCents,
          helpersCents,
    ...
    if (method === "POST" && path === "/api/bookings") {
      ...
      const vehicle = parsed(
        ...
        vehicle.data.pricePerKmCents === quote.data.vehicle.pricePerKmCents &&
          vehicle.data.helperPriceCents === quote.data.vehicle.helperPriceCents,
    ...
    if (match) {
      const booking = await bookingFor(user, match[1]),
        action = match[2];
      ...
      if (method === "POST" && action === "status") {
        const allowed =
          ...
          allowed[booking.status]?.includes(b.status),
          "Esta mudança de status não é permitida.",
          409,
        );
        if (b.status === "cancelado_cliente")
          ensure(
        ...
        await change(booking, b.status, user);
        return details(booking);
      }
      if (method === "POST" && action === "payment") {
        ...
        let payment = await one("SELECT * FROM payments WHERE booking_id=$1", [
          booking.id,
        ]);
        if (!payment) {
      ...
      if (method === "POST" && action === "review") {
        ...
        const rating = number(b.rating, "Nota", 1, 5, true),
          comment = text(b.comment || "", "Comentário", 0, 1000);
        await db.query(
    ...
    try {
      ...
      if (origin) {
        ...
        const sameOrigin = origin === `${forwardedProtocol}://${forwardedHost}`;
          ...
          config.origins.includes(origin) || sameOrigin,
      ...
      if (!path.startsWith("/api/")) {
        ...
        let file = resolve(
          ...
          file.startsWith(config.staticDir + sep),
          "Caminho inválido.",
          403,
        );
        try {
          ensure((await stat(file)).isFile(), "Arquivo não encontrado.", 404);
        } catch {
          throw new AppError(
            "Compile o Flutter Web para abrir a aplicação. Consulte o README.",
            404,
          );
        }
        const mime = {
      ...
      if (rateBuckets.size > 2000)
        for (const [k, v] of rateBuckets)
          if (v.until < Date.now()) rateBuckets.delete(k);
      ensure(
        bucket.count <= (authRoute ? 20 : 180),
      ...
      for await (const chunk of req) {
        ...
        ensure(bodyBytes <= 32768, "Solicitação muito grande.", 413);
        body += chunk;
      }
      let b = {};
      if (body) {
      ...
      const value = await db.transaction(async () =>
        route(req, path, b, isPublic ? null : await userFrom(req)),
      );
      ...
      const status =
        error.status ||
        (["23505", "SQLITE_CONSTRAINT_UNIQUE"].includes(error.code) ||
        ...
        JSON.stringify({
          error:
            status === 500
              ...
              : status === 409 && !error.status
                ? "Registro já existente ou reserva indisponível."
                : error.message,
          requestId,
        }),
      );
