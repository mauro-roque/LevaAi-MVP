...
test('mapas reais: busca pública e rota OSRM', { skip: process.env.RUN_MAPS_TEST !== 'true' }, async () => {
  ...
  assert(route.distanceKm > 0 && route.durationMinutes > 0 && route.coordinates.length > 2);
  console.log(`Rota real consultada: ${route.distanceKm} km, ${route.durationMinutes} minutos.`);
});
